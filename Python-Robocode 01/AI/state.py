...
class State:
    ...
    def eval(self):
        utility = 1 * (self.energy_self - self.energy_enemy)
    ...
    def apply_forward(self, enemy):
        if type(self.pos_enemy) == list:
            new_pos_enemy = [self.pos_enemy[0], self.pos_enemy[1]]
        else:
            new_pos_enemy = [self.pos_enemy.x(), self.pos_enemy.y()]
        if type(self.pos_self) == list:
            new_pos_self = [self.pos_self[0], self.pos_self[1]]
        else:
            new_pos_self = [self.pos_self.x(), self.pos_self.y()]
        new_shot_possibly_by_enemy = self.shot_possibly_by_enemy
        new_shot_possibly_at_enemy = self.shot_possibly_at_enemy
        if enemy:
