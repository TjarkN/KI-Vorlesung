...
class Action:
    ...
    __actions = {"turn_right": 45, "turn_left": -45, "forward": 10, "backward": -10, "shoot": 3}
    ...
    def faces(self, pos_1, pos_2, angle_1):
        if type(pos_1) == list:
            self_x, self_y = pos_1[0], pos_1[1]
        else:
            self_x, self_y = pos_1.x(), pos_1.y()
        if type(pos_2) == list:
            enemy_x, enemy_y = pos_2[0], pos_2[1]
        else:
            enemy_x, enemy_y = pos_2.x(), pos_2.y()
        dir_enemy = enemy_x - self_x, enemy_y - self_y
        if dir_enemy == (0,0):
            return False
    ...
    @classmethod
    def x_distance_between(self, pos_1, pos_2):
        if type(pos_1) == list:
            self_x, self_y = pos_1[0], pos_1[1]
        else:
            self_x, self_y = pos_1.x(), pos_1.y()
        if type(pos_2) == list:
            ...
            enemy_x, enemy_y = pos_2.x(), pos_2.y()
        dir_enemy = enemy_x - self_x
        return dir_enemy

    @classmethod
    def y_distance_between(self, pos_1, pos_2):
        if type(pos_1) == list:
            self_x, self_y = pos_1[0], pos_1[1]
        else:
            self_x, self_y = pos_1.x(), pos_1.y()
