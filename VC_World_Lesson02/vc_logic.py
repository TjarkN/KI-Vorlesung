...
class VCNew(VC):
    ...
    def is_goal_state(self, state):
        # todo: Wenn alle goal states sind, dann!
        if len(state) == 1:
            return super().is_goal_state(state[0])

    def get_applicable_actions(self, state):
        actions = {}
        for s in state:
            act_actions = set(super().get_applicable_actions(s))
            actions = actions.union(act_actions)
        return list(actions)

    def perform_action(self, state, action):
        new_state = []
        for s in state:
            s_new = super().perform_action(s, action)
            if s.to_tate() is not s_new.to_state():
                new_state.append(s)
        return new_state
...
class Environment:
    ...
    def __init__(self, size=(3, 3), dirt_rate=0.6, width_room=274, height_room=240, seed_value=1, random_action_rate=0,
                 perception=False):

        if seed_value is not None:
            seed(seed_value)
        self.size = size
        self.building = Building(self.size, dirt_rate, seed=seed_value)
        if perception:
            self.vc = VCBS(choice([[x, y] for x in range(self.size[0]) for y in range(self.size[1])]), self.building,
                           random_action_rate=random_action_rate, perception=perception)
        else:
            self.vc = VC(choice([[x, y] for x in range(self.size[0]) for y in range(self.size[1])]), self.building,
