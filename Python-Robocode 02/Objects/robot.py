...
from Objects.radarField import radarField
from Objects.animation import animation
from math import floor
from AI.actions import Action


class Robot(QtWidgets.QGraphicsItemGroup):
    
    def __init__(self,  mapSize, parent, repr, identity, model = None, training = None):
        ...
        #load img
        self.__base = QtWidgets.QGraphicsPixmapItem()
        ...
        #load gun img
        self.__gun = QtWidgets.QGraphicsPixmapItem()
        ...
        #load radar img
        self.__radar = QtWidgets.QGraphicsPixmapItem()
        ...
        if self.__currentAnimation == []:
            try:
                ...
                if self.__physics.animation.name == "target":
                    ...
                    self.stop()
                    self.run()
                    self.__parent.add_action_counter()
            ...
            for item in set(self.__base.collidingItems(1)) - self.__items:
                if isinstance(item, QtWidgets.QGraphicsRectItem):
                    ...
                    if item.robot.__physics.animation.name != "target":
                        #targetSpotted
                        if not self.__target_seen:
                            self.__targetSeen(item)
        
     ### THESE ARE THE FUNCTIONS ACCESSABLE FROM OUTSIDE ###   

     #-----------------------------------------------------------Gun------------------------------------------------------
    def gun_to_side(self):
        self.__gun.setRotation(90)
    ...
    def gunTurn(self, angle):
        ...
        a = (s*angle)%self.__physics.step
        if a != 0:
            self.__physics.gunTurn.append(s*a)
        for i in range(floor(steps)):
    ...
    def forward(self):
        self.move(Action.get_forward())
        self.pause(Action.get_pause())

    def backwards(self):
        self.move(Action.get_backward())
        self.pause(Action.get_pause())

    def turn_right(self):
        self.turn(Action.get_turn_right())
        self.gunTurn(Action.get_turn_right())
        self.pause(Action.get_pause())

    def turn_left(self):
        self.turn(Action.get_turn_left())
        self.gunTurn(Action.get_turn_left())
        self.pause(Action.get_pause())

    def shoot(self):
        self.fire(Action.get_shoot())
        self.pause(Action.get_pause())
    ...
    def shot_possible_at_enemy(self):
        if len(self.__parent.aliveBots) > 1:
            enemy = self.__parent.aliveBots[0]
            if enemy == self:
                enemy = self.__parent.aliveBots[1]
            return Action.faces(self.getPosition(), enemy.getPosition(), self.getGunHeading())

    def shot_possible_by_enemy(self):
        if len(self.__parent.aliveBots) > 1:
            enemy = self.__parent.aliveBots[0]
            if enemy == self:
                enemy = self.__parent.aliveBots[1]
            return enemy.shot_possible_at_enemy()

    def energy_left_self(self):
        return self.__health

    def energy_left_enemy(self):
        if len(self.__parent.aliveBots) > 1:
            enemy = self.__parent.aliveBots[0]
            if enemy == self:
                enemy = self.__parent.aliveBots[1]
            return enemy.energy_left_self()

    def getPosition_enemy(self):
        if len(self.__parent.aliveBots) > 1:
            enemy = self.__parent.aliveBots[0]
            if enemy == self:
                enemy = self.__parent.aliveBots[1]
    ...
    def getGunHeading_enemy(self):
        if len(self.__parent.aliveBots) > 1:
            enemy = self.__parent.aliveBots[0]
            if enemy == self:
                enemy = self.__parent.aliveBots[1]
            return enemy.getGunHeading()
    ...
    def move(self, distance):
        ...
        d = (s*distance)%self.__physics.step
        if d != 0:
    ...
    def slide(self, direction): #moves 5 units in direction
        if direction:
            self.move(*5)
        else:
            self.move(-5)

    def turn_to_enemy(self):
       if len(self.__parent.aliveBots) > 1:
            enemy = self.__parent.aliveBots[0]
            if enemy == self:
                enemy = self.__parent.aliveBots[1]
            ...
            dir_enemy = enemypos - mypos;
            dir_enemy = (dir_enemy.x(), dir_enemy.y())
            heading_self = (self.getHeading() / 360 - self.getHeading() // 360) * 360
            heading_self_r = heading_self * math.pi / 180
            dir_self = (-math.sin(heading_self_r), math.cos(heading_self_r))
            angle_enemy = Action.angle_between(dir_self, dir_enemy)
            ...
            if angle_enemy != 0:
                ...
                if heading_self == 0:
                    if enemypos.x() >= mypos.x():
                        direct = -1
                elif heading_self == 180:
                    if enemypos.x() <= mypos.x():
                        direct = -1
                else:
                    alpha = 1.5 * math.pi - heading_self_r
                    if heading_self_r > 1.5 * math.pi:
                        alpha = 2 * math.pi + alpha
                    a = -math.tan(alpha)
                    b = mypos.y() - a * mypos.x()
                    if heading_self >= 180 and a * enemypos.x() + b >= enemypos.y():
                        direct = -1
                    elif heading_self <= 180 and a * enemypos.x() + b <= enemypos.y():
                        direct = -1

                rotation_left = 0.0
                for animation_list in self.__physics.animation.list:
                    for a in animation_list:
                        rotation_left += a["turn"]
                angle = (direct * angle_enemy - rotation_left) - direct * 90
                if angle != 0:
                    self.turn(angle)
    ...
    def aim_at_enemy(self):
       if len(self.__parent.aliveBots) > 1:
            ...
            dir_enemy = enemypos-mypos;
            dir_enemy = (dir_enemy.x(), dir_enemy.y())
            heading_self = (self.getGunHeading() / 360 - self.getGunHeading() // 360) * 360
            heading_self_r = heading_self * math.pi / 180
            dir_self = (-math.sin(heading_self_r), math.cos(heading_self_r))
            angle_enemy = Action.angle_between(dir_self, dir_enemy)
            angle_enemy = 180/math.pi * angle_enemy
            ...
            if angle_enemy != 0:
                ...
                gun_rotation_left = 0.0
                for animation_list in self.__physics.animation.list:
                    for a in animation_list:
                        gun_rotation_left += a["gunTurn"]
                gun_angle = direct * angle_enemy - gun_rotation_left
                if gun_angle != 0:
                    self.gunTurn(gun_angle)
            if -2 <= angle_enemy <= 2:
                self.set_target_seen(True)

    def get_next_Command_executed(self):
        return self.__next_command_executed

    def set_next_command_executed(self, executed):
        self.__next_command_executed = executed

    def get_target_seen(self):
        return self.__target_seen

    def set_target_seen(self, seen):
        self.__target_seen = seen
    ...
    def getMapSize(self):
        return (self.__mapSize.width(), self.__mapSize.height())
    ...
    def __death(self):
        ...
        enemy_training = False
        for enemy in self.__parent.aliveBots:
            if enemy.training is not None:
                enemy_training = True
            enemy.onEnemyDeath()
        self.onRobotDeath()
        self.__parent.removeItem(self)
        if  len(self.__parent.aliveBots) <= 1 and self.training is None and not enemy_training:
