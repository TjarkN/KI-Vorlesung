...
:- dynamic fluent/1.
fluent(energy(self,100.0)).
fluent(energy(enemy,100.0)).
fluent(pos(self,[0.0,0.0])).
%fluent(orientation(self,O)).
...
poss(move_forward(X), S) :-
    holds(pos(X,[_,_]), S).

poss(move_backward(X), S) :-
    holds(pos(X,[_,_]), S).
...
result(S1, move_forward(X), S2) :-
    holds(pos(X,[R,L]), S1),
    fact(move_range(V)),
	RplusV is R + V,
    replace_fluent(S1, pos(X,[R,L]),
	pos(X,[RplusV,L]), S2).

result(S1, move_backward(X), S2) :-
    holds(pos(X,[R,L]), S1),
    fact(move_range(V)),
	RminusV is R - V,
    replace_fluent(S1, pos(X,[R,L]),
...
goal(energy(self,105.0)).
goal(pos(self,[-50.0,50.0])).
