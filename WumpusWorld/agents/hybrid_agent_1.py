...
class KnowledgeBase:
    ...
    def resolution(self, clause=None):
        # negieren der Aussage (für einen fall)
        neg_clause = []

        for element in clause:
            temp_element = list(element)
            temp_element[0] = not temp_element[0]
            temp_element = tuple(temp_element)
            neg_clause.append(temp_element)

        temp_kb = self.KB.copy()

        # widerspruch in kb einfügen
        for signed_symbol in neg_clause:
            if signed_symbol[1] not in temp_kb.keys():
                temp_kb[(True, signed_symbol[1])] = []
                temp_kb[(False, signed_symbol[1])] = []
            if neg_clause not in temp_kb[signed_symbol]:
                temp_kb[signed_symbol].append(neg_clause)

        new = []
        for key in temp_kb:
            for item in temp_kb[key]:
                new.append(item)
        copy_new = deepcopy(new)
        resolvements = []

        for ci in new:
            for cj in new:
                # bool in tuple drehen
                for element in cj:
                    temp_element = list(element)
                    temp_element[0] = not temp_element[0]
                    temp_element = tuple(temp_element)
                    if temp_element in ci:
                        resolvements.append(ci.remove(temp_element))
                        if not resolvements[-1]:
                            print("juhu")
                            return True

        #fehlt: abfrage nach auflösung der passenden klausel der eingabe
        #vereinigung von resolvements und new-liste, danach dauerschleife
        #solange durchziehen, bis eingegebene klausel gelöst werden kann
        #hier: (False, "Gold_x_y") mit (True, "Gold_x_y") -> [] --> return True
...
def set_up_ww(size):
    ...
    for x in range(size[0]):
        for y in range(size[1]):
            ...
            """
            When we percept no glitter there is no gold on the current field
            not Glitter_t and Agent_X_Y_t => not Gold_X_Y --> Glitter_t or not Agent or not Gold
            """
            #fluent = []
            #fluent.append((True, f"Glitter_#t"))
            #fluent.append((False, f"Agent_{x}_{y}_#t"))
            #fluent.append((False, f"Agent_{x}_{y}_#t"))
            #ww_KB.add_fluent(fluent)
